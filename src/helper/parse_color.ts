...
function clamp_css_byte(i: number): number {
    // Clamp to integer 0 .. 255.
    i = Math.round(i); // Seems to be what Chrome does (vs truncation).
    return i < 0 ? 0 : i > 255 ? 255 : i;
}

function clamp_css_float(f: number): number {
    // Clamp to float 0.0 .. 1.0.
    return f < 0 ? 0 : f > 1 ? 1 : f;
}

function parse_css_int(str: string): number {
    // int or percentage.
    if (str[str.length - 1] === "%")
...
function parse_css_float(str: string): number {
    // float or percentage.
...
function parseCSSColor(css_str: string): RGBA | null {
    ...
    if (str[0] === "#") {
        ...
        for (let i = 0; i < until; i++) {
            ...
            result[i] =
                ...
                (is_short
                    ? parsed
                    : parse_hex_charcode(str.charCodeAt(i * mult + 2)));
    ...
    if (op !== -1 && (ep + 1 === str.length || ep === str.length)) {
        ...
        switch (fname) {
            ...
            case "hsl":
                h =
                    (((rotation_to_degrees(params[0]) % 360) + 360) % 360) /
                    360; // 0 .. 1
